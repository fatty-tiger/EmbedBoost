...
# BertForMaskedLM.gradient_checkpointing_enable()


class RetroMAEForPretraining(nn.Module):
    def __init__(
            self,
            bert: Union[BertForMaskedLM, ErnieForMaskedLM],
            model_args: ModelArguments,
    ):
        super(RetroMAEForPretraining, self).__init__()
        self.lm = bert
        print("lm_config:", self.lm.config)
        
        if isinstance(bert, BertForMaskedLM):
            self.embeddings = self.lm.bert.embeddings
            self.decoder_embeddings = self.lm.bert.embeddings
    ...
    def forward(self,
        ...
        # decoder_position_ids = self.lm.bert.embeddings.position_ids[:, :decoder_input_ids.size(1)]
        # decoder_position_embeddings = self.lm.bert.embeddings.position_embeddings(decoder_position_ids)  # B L D
        decoder_position_ids = self.embeddings.position_ids[:, :decoder_input_ids.size(1)]
