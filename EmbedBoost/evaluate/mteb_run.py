...
class BgeM3Model:
    def __init__(self) -> None:
        ...
        self.model = FlagModel('/home/public/pretrained_models/bge-small-zh-v1.5', devices=['cuda:0'])
...
class CustomModel:
    def __init__(self) -> None:
        ...
        #model_name_or_path = "output/multicpr_med/v1/ckp_epoch_20_step_1300/q_model"
        # model_name_or_path = "output/multicpr_med/v2/ckp_epoch_60_step_720/q_model"
        use_dense = True
        dense_pooling = "cls"
        dense_dim = 128
        infer_dense_dim = 128
        ...
        self.embedder = embedder = BGEM3Embedder(
            ...
            dense_dim=dense_dim,
            infer_dense_dim=infer_dense_dim,
    ...
    def encode(self, sentences, batch_size=32, **kwargs) -> np.ndarray:
        ...
        Returns:
            The encoded sentences.
        """
        res = self.embedder.encode(sentences, batch_size=batch_size, max_length=512)
...
def main():
    ...
    results = evaluator.run(model, verbosity=2, output_folder=f"output/mteb_MedicalRetrieval_{ts}", encode_kwargs=encode_kwargs)
    for res in results:
    #     print(res.task_name)
